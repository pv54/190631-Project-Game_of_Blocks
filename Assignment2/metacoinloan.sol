...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    function getOwnerBalance() public view returns(uint256) {
     ...
     function viewDues(address from_name) public view isOwner returns(uint256) {
		return loans[from_name];
    }
    function settleDues(address from_name) public  isOwner returns(bool) {
		    bool check=sendCoin(from_name,loans[from_name],Owner);
		    if(check)
		    {loans[from_name]-=loans[from_name]};
		    return check;
	}
